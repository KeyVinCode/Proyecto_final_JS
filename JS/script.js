document.addEventListener('DOMContentLoaded', () => {
    ...
    // Cerrar el desplegable al hacer clic fuera del botón flotante
    document.addEventListener('click', function (e) {
      const float = document.getElementById('social-float');
...
// ── Toggle desplegable de redes sociales ──────────────────────────────────────
function toggleSocial() {
  const dropdown = document.getElementById('social-dropdown');
  const chevron  = document.getElementById('chevron-icon');
  if (dropdown && chevron) {
      const isOpen   = dropdown.classList.toggle('open');
      // Rotar el chevron según el estado
